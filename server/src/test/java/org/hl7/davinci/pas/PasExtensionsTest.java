...
class PasExtensionsTest {
  ...
  void buildReviewActionExtension_certifiedInTotal() {
    Extension ext = PasExtensions.buildReviewActionExtension(
        PasExtensions.REVIEW_CODE_A1, "Certified in total", "AUTH001");

    assertEquals(PasExtensions.REVIEW_ACTION, ext.getUrl());

    Extension codeExt = ext.getExtensionByUrl(PasExtensions.REVIEW_ACTION_CODE);
    assertNotNull(codeExt);
    CodeableConcept cc = (CodeableConcept) codeExt.getValue();
    assertEquals("A1", cc.getCodingFirstRep().getCode());
    assertEquals(PasExtensions.X12_REVIEW_CODE_SYSTEM, cc.getCodingFirstRep().getSystem());
  ...
  void buildReviewActionExtension_pendedNoAuthNumber() {
    Extension ext = PasExtensions.buildReviewActionExtension(
        PasExtensions.REVIEW_CODE_A4, "Pending", null);
  ...
  void buildPreAuthPeriodExtension_withDates() {
    ...
    assertEquals(PasExtensions.ITEM_PREAUTH_PERIOD, ext.getUrl());
